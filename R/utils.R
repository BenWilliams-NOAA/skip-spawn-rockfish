...
plot_risk <- function(sims, rmv_yrs = 15) {
  # manager's view
  sims %>%
    mutate(scenario = ifelse(scenario == "i", 
                             "Mgmt: Functional (Correct)", 
...
plot_ssb_shock <- function(rpt, sims, ssb_type = "spawn_bio_f") {
  bind_rows(data.frame(year = rpt$years, 
                       spawn_bio = rpt[[ssb_type]], 
                       scenario = "historical",
                       iteration = 0),
            data.frame(year = max(rpt$years) + sims$year,
                       spawn_bio = sims[[ssb_type]], 
                       # type = "projection", 
                       scenario = sims$scenario,
                       iteration = sims$iteration)) %>% 
...
table_risk <- function(sims, rmv_yrs = 15) {
  sims %>%
    filter(year > (min(year) + rmv_yrs)) %>% 
    left_join(
      s1 %>% filter(scenario == "i") %>% 
        select(iteration, year, TRUE_B35 = B35_f, TRUE_F35 = F35_f),
      by = c("iteration", "year")) %>%
    mutate(x_ratio = spawn_bio_f / TRUE_B35,
           y_ratio = Ft / TRUE_F35,
           zone = case_when(
             x_ratio < 1 & y_ratio > 1  ~ "Red (Overfished & Overfishing)",
             x_ratio > 1 & y_ratio < 1  ~ "Green (Safe)",
             x_ratio < 1 & y_ratio <= 1 ~ "Yellow (Overfished only)",
             x_ratio >= 1 & y_ratio >= 1 ~ "Yellow (Overfishing only)")) %>%
