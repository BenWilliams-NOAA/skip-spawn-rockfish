...
# recruitment matrix
collapse = 15 # number of years of population collapse (if desired)
set.seed(309)
...
# save results
plot_risk(s3, ssb_col = "spawn_bio_r", bio_ref_col = "B35", F_col = "F35", rmv_yrs = 0) +
  ggtitle("Manager's View")# mgmt view
plot_risk(s3, rmv_yrs = 0) # reality view
table_risk(s3, ssb_col = "spawn_bio_r", bio_ref_col = "B35", F_col = "F35", rmv_yrs = 0) # mgmt view
table_risk(s3, rmv_yrs = 0)
plot_F_shock(rpt, s3)
plot_ssb_shock(rpt, s3)
plot_catch(rpt, s3)
plot_recruits(rpt, s3)
